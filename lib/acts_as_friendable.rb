...
module ActsAsFriendable
  def self.included(receiver)
    receiver.class_eval do
      ...
      has_many :direct_friends, :through => :friendships, :conditions => ["status = ?", "accepted"], :source => :friend
      has_many :inverse_friends, :through => :inverse_friendships, :conditions => ["status = ?", "accepted"], :source => :user
      has_many :pending_friends, :through => :friendships, :conditions => ["status = ?", "pending"], :source => :friend
      has_many :requested_friendships, :through => :inverse_friendships, :conditions => ["status = ?", "pending"], :source => :user
    end
